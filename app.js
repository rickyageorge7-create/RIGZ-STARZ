...
const supabaseConfig = window.RIGZA_SUPABASE_CONFIG || {};
const supabaseClient = window.supabase && supabaseConfig.url && supabaseConfig.anonKey
  && !supabaseConfig.url.includes('YOUR_PROJECT') && !supabaseConfig.anonKey.includes('YOUR_PUBLIC')
...
async function loadCloudState(user) {
  ...
  if (data) state = mergeState({ ...data.state, profile: data.profile });
  else {
    state = structuredClone(DEFAULT_STATE);
    state.profile = { name: user.user_metadata?.full_name || '', email: user.email || '' };
    await saveState();
  }
}
...
function renderDashboard() {
  ...
  } else if (!state.classes.length && !state.assignments.length) {
    todayStatus.textContent = 'Welcome to RIGZA 👋';
    todayDetail.textContent = 'Let’s get your student workspace ready. Add your first class or assignment to begin.';
  } else {
  ...
  document.getElementById('nextClass').innerHTML = next ? `<div class="item"><strong>${next.cls.code} — ${next.cls.name}</strong><span class="meta">${next.day}, ${fmtTime(next.cls.start)}–${fmtTime(next.cls.end)} · ${next.cls.room}</span></div>` : state.classes.length ? '<div class="empty">No upcoming class found.</div>' : '<div class="empty">Welcome to your workspace. Add your first class from Schedule.</div>';
  const dueSoon = upcomingAssignments().slice(0, 4);
  document.getElementById('dueSoon').innerHTML = dueSoon.length ? dueSoon.map(renderAssignmentItem).join('') : state.assignments.length ? '<div class="empty">No assignments due soon.</div>' : '<div class="empty">No assignments yet. Add one when you are ready.</div>';
...
function renderPulse(now, checked, first) {
  ...
  if (!state.classes.length && !state.assignments.length) {
    headline = 'Welcome to RIGZA 👋';
    message = 'Let’s get your student workspace ready. Add a class, assignment, or profile detail to make this dashboard yours.';
    nextMove = 'Add your first class';
  }
...
function copilotReply(question) {
  ...
  if (identityQuestion) {
    return hasAccount
      ...
      : 'You are signed out. Sign in to use your private student workspace.';
  }
  if (greeting) {
    return hasAccount
      ? `Hi ${studentName}! I can help with your schedule, assignments, check-ins, progress, settings, or questions about RIGZA.`
      : 'Hi! Sign in to RIGZA and I can help with your schedule, assignments, check-ins, and progress.';
  }
...
async function unlockApp() {
  ...
  function setAuthMode(signUp) {
    ...
    document.getElementById('authHint').textContent = signUp ? 'Use your account to keep your schedule, assignments, progress, and profile safely separated.' : 'Your schedule, assignments, progress, and profile stay connected to your account.';
    document.getElementById('authNameLabel').hidden = !signUp;
    ...
    document.getElementById('authModeToggle').textContent = signUp ? 'Back to sign in' : 'Create an account';
    document.getElementById('authModeToggle').hidden = false;
    ...
    document.getElementById('cancelPasswordReset').hidden = true;
    setAuthMessage('');
  }
  function setPasswordRecoveryMode() {
    document.getElementById('authTitle').textContent = 'Choose a new password';
    document.getElementById('authEyebrow').textContent = 'Secure account recovery';
    document.getElementById('authHint').textContent = 'Set a new password for your RIGZA account.';
    ...
    document.getElementById('authModeToggle').hidden = true;
    document.getElementById('forgotPassword').hidden = true;
    document.getElementById('cancelPasswordReset').hidden = false;
    setAuthMessage('');
  }
  function showAuthGate(visible) {
    ...
    const copilot = document.getElementById('copilotToggle');
    gate.hidden = !visible;
    shell.hidden = visible;
    mobileNav.hidden = visible;
    copilot.hidden = visible;
    gate.style.display = visible ? 'grid' : 'none';
    shell.style.display = visible ? 'none' : '';
    mobileNav.style.display = visible ? 'none' : '';
    copilot.style.display = visible ? 'none' : '';
  }
  async function handleAuthSubmit(event) {
    event.preventDefault();
    if (!supabaseClient) return setAuthMessage('Add your Supabase URL and public anon key to supabase-config.js first.', true);
    const signUp = !document.getElementById('authNameLabel').hidden;
  ...
  async function initializeAuthentication() {
    if (!supabaseClient) {
      showAuthGate(true);
      setAuthMessage('Supabase is not configured yet. Add the public values in supabase-config.js.', true);
      return;
    }
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
      ...
      try {
        ...
        showAuthGate(true);
        setAuthMessage(`Could not load your workspace: ${error.message}`, true);
      }
    });
}
...
async function updateProfile() {
  ...
  state.profile = { name, email: currentUser.email || profileEmail.value.trim() };
  await saveState();
  render();
...
setTimeout(() => { if (currentUser) checkReminders(); }, 1500);
...
document.getElementById('createAccount').addEventListener('click', updateProfile);
...
document.getElementById('cancelPasswordReset').addEventListener('click', () => setAuthMode(false));
setAuthMode(false);
